...
class Task:
    def __init__(self, title, deadline=None, priority="Medium",
                 category="General", done=False, task_id=None):
...
class TaskManager:
    ...
    def load(self):
        try:
            ...
            for t in data.get("tasks", []):
                ...
                task.created_at = t.get("created_at")  # ← FIX 2 (safe)
                self.tasks.append(task)
    ...
    def mark_done_by_id(self, task_id, done=True):
        for t in self.tasks:
            if t.id == task_id:
                t.done = done
                self.save()
    ...
    def move_task_category(self, task_id, new_category):
        for t in self.tasks:
            if t.id == task_id:
